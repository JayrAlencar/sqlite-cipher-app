...
app.controller("mainController", function($scope, $controller, databaseService){
	...
	$scope.disconnect = function(data){
		databaseService.disconnect(data);
		$scope.$broadcast('newConnection',[]);
		$scope.getDatabases();
	}
	...
	$scope.functions = function(base){
		$('#modalFunctions').modal('show');
		$scope.$broadcast('databaseFun',base);
	}
